...
class ClientLauncher {
    static launch(aServerURL, aWebSockURL, aWsAPI) {
        ...
        // Sets up web socket
        // Incoming message is just handled as a Reset, i.e. all counters re-rendered
        const socket = new WebSocket(aWebSockURL);
        socket.addEventListener('message', event => { handlerReset(JSON.parse(event.data)); });
    }
}


var theWsAPI = new WebServiceAPI("/api");
var localWebSockURL =  'ws://localhost:1337';
var localServerURL = 'http://127.0.0.1:1337';
var azureServerURL = 'http://roleplayonazure.azurewebsites.net';

window.onload = function() {
    ClientLauncher.launch(localServerURL, localWebSockURL, theWsAPI);
}
